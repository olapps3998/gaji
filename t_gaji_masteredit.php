...
$t_gaji_master_edit = NULL; // Initialize page object first

class ct_gaji_master_edit extends ct_gaji_master {
	...
	// Page object name
	var $PageObjName = 't_gaji_master_edit';
	...
	function Page_Main() {
		...
		if (@$_GET["gjm_id"] <> "") {
			...
			$this->RecKey["gjm_id"] = $this->gjm_id->QueryStringValue;
		} else {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("t_gaji_masterlist.php"); // Return to list page
			...
			while (!$this->Recordset->EOF) {
				if (strval($this->gjm_id->CurrentValue) == strval($this->Recordset->fields('gjm_id'))) {
	...
	function LoadFormValues() {
		...
		if (!$this->gjm_id->FldIsDetailKey)
			$this->gjm_id->setFormValue($objForm->GetValue("x_gjm_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->gjm_id->CurrentValue = $this->gjm_id->FormValue;
	...
	function ValidateForm() {
		...
		if (in_array("t_gaji_detail", $DetailTblVar) && $GLOBALS["t_gaji_detail"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// prj_id
			$this->prj_id->SetDbValueDef($rsnew, $this->prj_id->CurrentValue, 0, $this->prj_id->ReadOnly);

			// per_id
			$this->per_id->SetDbValueDef($rsnew, $this->per_id->CurrentValue, 0, $this->per_id->ReadOnly);

			// gjm_total
			$this->gjm_total->SetDbValueDef($rsnew, $this->gjm_total->CurrentValue, NULL, $this->gjm_total->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("t_gaji_detail", $DetailTblVar) && $GLOBALS["t_gaji_detail"]->DetailEdit) {
						if (!isset($GLOBALS["t_gaji_detail_grid"])) $GLOBALS["t_gaji_detail_grid"] = new ct_gaji_detail_grid(); // Get detail page object
						$EditRow = $GLOBALS["t_gaji_detail_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_gaji_detail", $DetailTblVar)) {
				...
				if ($GLOBALS["t_gaji_detail_grid"]->DetailEdit) {
					$GLOBALS["t_gaji_detail_grid"]->CurrentMode = "edit";
					$GLOBALS["t_gaji_detail_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($t_gaji_master_edit)) $t_gaji_master_edit = new ct_gaji_master_edit();

// Page init
$t_gaji_master_edit->Page_Init();

// Page main
$t_gaji_master_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_gaji_master_edit->Page_Render();
?>
...
var CurrentForm = ft_gaji_masteredit = new ew_Form("ft_gaji_masteredit", "edit");

// Validate form
ft_gaji_masteredit.Validate = function() {
...
ft_gaji_masteredit.Form_CustomValidate = 
...
ft_gaji_masteredit.ValidateRequired = true;
<?php } else { ?>
ft_gaji_masteredit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_gaji_masteredit.Lists["x_prj_id"] = {"LinkField":"x_prj_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_prj_nama","x_prj_lokasi","x_prj_pekerjaan",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_project"};
ft_gaji_masteredit.Lists["x_per_id"] = {"LinkField":"x_per_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_per_mulai","x_per_selesai","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_periode"};
...
<?php if (!$t_gaji_master_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_gaji_master_edit->ShowPageHeader(); ?>
<?php
$t_gaji_master_edit->ShowMessage();
?>
<?php if (!$t_gaji_master_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_gaji_master_edit->Pager)) $t_gaji_master_edit->Pager = new cPrevNextPager($t_gaji_master_edit->StartRec, $t_gaji_master_edit->DisplayRecs, $t_gaji_master_edit->TotalRecs) ?>
<?php if ($t_gaji_master_edit->Pager->RecordCount > 0 && $t_gaji_master_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_gaji_master_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_gaji_master_edit->PageUrl() ?>start=<?php echo $t_gaji_master_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_gaji_master_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_gaji_master_edit->PageUrl() ?>start=<?php echo $t_gaji_master_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_gaji_master_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_gaji_master_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_gaji_master_edit->PageUrl() ?>start=<?php echo $t_gaji_master_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_gaji_master_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_gaji_master_edit->PageUrl() ?>start=<?php echo $t_gaji_master_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_gaji_master_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="ft_gaji_masteredit" id="ft_gaji_masteredit" class="<?php echo $t_gaji_master_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_gaji_master_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_gaji_master_edit->Token ?>">
<?php } ?>
...
<input type="hidden" data-table="t_gaji_master" data-field="x_gjm_id" name="x_gjm_id" id="x_gjm_id" value="<?php echo ew_HtmlEncode($t_gaji_master->gjm_id->CurrentValue) ?>">
<?php
	if (in_array("t_gaji_detail", explode(",", $t_gaji_master->getCurrentDetailTable())) && $t_gaji_detail->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_gaji_master_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($t_gaji_master_edit->Pager)) $t_gaji_master_edit->Pager = new cPrevNextPager($t_gaji_master_edit->StartRec, $t_gaji_master_edit->DisplayRecs, $t_gaji_master_edit->TotalRecs) ?>
<?php if ($t_gaji_master_edit->Pager->RecordCount > 0 && $t_gaji_master_edit->Pager->Visible) { ?>
...
<script type="text/javascript">
ft_gaji_masteredit.Init();
</script>
<?php
$t_gaji_master_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_gaji_master_edit->Page_Terminate();
?>
